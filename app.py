...
class RelayWorker(threading.Thread):
    ...
    def request_restart(self, reason: str = "تغيير الرسوم") -> None:
        """يطلب من العامل إنهاء الجلسة بلطف وإعادة البناء (تُسد الفجوة بشاشة الاستراحة)."""
    ...
    def run(self) -> None:
        ...
        while not self.stop_event.is_set():
            ...
            try:
                ...
                while not self.stop_event.is_set():
                    try:
                        ...
                        if self._restart_evt.is_set():
                            ...
                            reason = self._restart_reason or "تغيير من اللوحة"
                            self._restart_reason = None
                            restart_by_panel = True
                            LOGGER.info("[%s] إعادة تشغيل مطلوبة من اللوحة (%s) — بث الاستراحة يغطي الفجوة.", self.name_value, reason)
                            terminate_process(proc)
                            try:
                ...
                if refreshing and not restart_by_panel:
                    # تجديد دوري لمصدر حي — لا حاجة لشاشة الاستراحة
                    pass
                else:
                    # انقطاع/فشل/طلب لوحة: شاشة الاستراحة تحفظ البث من القطع
                    self._ensure_break()
                self.wait_before_retry(retry_delay)
...
class RelayManager:
    ...
    def apply_graphics_now(self, changes: dict[str, Any]) -> tuple[bool, str, list[str]]:
        """تغيير رسوم فوري من اللوحة: يُدمج في الإعدادات ويُعاد بناء البث عبر جسر الاستراحة."""
        changed = apply_config_merge(self.config, changes)
        if changed:
            self.config.save()
...
def apply_config_merge(config: RelayConfig, changes: dict[str, Any]) -> list[str]:
    """يدمج تغييرات جزئية في كائن الإعدادات نفسه (ليراه العمال دون استبدال مرجع) ويعيد المفاتيح المتغيرة."""
    merged = {**asdict(config), **changes}
    candidate = RelayConfig.from_dict(merged)
    changed: list[str] = []
...
function gfxAct(changes){return api('/api/gfx',{method:'POST',body:JSON.stringify(changes)}).then(d=>notice(d.message)).catch(e=>notice(e.message,'error')).finally(()=>refresh())}
function renderLiveCtl(c){let box=$('liveCtl');if(!box)return;let btns=[];if(c&&c.logo_path){btns.push(`<button class="btn wbtn" data-act='{"logo_mode":"always"}'>شعار: إظهار</button>`);btns.push(`<button class="btn wbtn" data-act='{"logo_mode":"off"}'>شعار: إخفاء</button>`)};(c?.pip_slots||[]).forEach((p,i)=>{let nm=esc(p.name||('عرض '+(i+1)));btns.push(`<button class="btn wbtn" data-act='${JSON.stringify({["pip_slots"]:(c.pip_slots.map((x,j)=>j===i?{...x,mode:"always"}:x))})}'>${nm}: إظهار</button>`);btns.push(`<button class="btn wbtn" data-act='${JSON.stringify({["pip_slots"]:(c.pip_slots.map((x,j)=>j===i?{...x,mode:"off"}:x))})}'>${nm}: إخفاء</button>`)});if(!btns.length){box.innerHTML='<span class="label">فعّل شعاراً أو منافذ عرض من الإعدادات لتظهر أزرار التحكم السريع هنا.</span>';return}box.innerHTML=btns.join('');box.querySelectorAll('[data-act]').forEach(b=>b.onclick=()=>gfxAct(JSON.parse(b.dataset.act)))}
async function refresh2(){try{let [s,l]=await Promise.all([api('/api/status'),api('/api/logs')]);renderStatus(s);let box=$('logs'),nearBottom=box.scrollHeight-box.scrollTop-box.clientHeight<40;box.textContent=(l.lines||[]).join('\n')||'لا يوجد سجل بعد.';if(nearBottom)box.scrollTop=box.scrollHeight;renderLiveCtl(lastConfig)}catch(e){notice(e.message,'error')}}
        ...
        try:
            ...
            if path == "/api/gfx":
                fields = {k: v for k, v in data.items() if k in {
                    "logo_path", "logo_position", "logo_width", "logo_mode",
